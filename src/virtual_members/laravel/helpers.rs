...
/// How many directory levels up from `__DIR__` an expression names, e.g.
/// `__DIR__` itself is 0 levels up, `dirname(__DIR__)` is 1, and
/// `dirname(__DIR__, 2)` (or nested `dirname(dirname(__DIR__))`) is 2.
///
/// Returns `None` for anything that doesn't ultimately bottom out at
/// `__DIR__`.
fn dir_levels_up(expr: &Expression<'_>) -> Option<u32> {
    match expr {
        Expression::MagicConstant(MagicConstant::Directory { .. }) => Some(0),
        Expression::Parenthesized(p) => dir_levels_up(p.expression),
        Expression::Call(Call::Function(fc)) => {
            let Expression::Identifier(id) = fc.function else {
                return None;
            };
            let name = crate::atom::bytes_to_str(id.value()).trim_start_matches('\\');
            if !name.eq_ignore_ascii_case("dirname") {
                return None;
            }
            let args: Vec<_> = fc.argument_list.arguments.iter().collect();
            let inner_levels = dir_levels_up(args.first()?.value())?;
            let extra = match args.get(1) {
                Some(levels_arg) => match levels_arg.value() {
                    Expression::Literal(literal::Literal::Integer(int_lit)) => {
                        int_lit.value.unwrap_or(1) as u32
                    }
                    _ => return None,
                },
                None => 1,
            };
            Some(inner_levels + extra)
        }
        _ => None,
    }
}
...
/// also following `dirname(__DIR__) . '/path.php'` (and its multi-level
/// forms, `dirname(__DIR__, 2)` / nested `dirname()` calls) for a provider
/// that sits below the package root.
///
/// Returns how many directory levels up from `__DIR__` the concatenation is
/// anchored, along with the string literal portion (e.g. `"/routes/api.php"`).
pub(crate) fn extract_dir_concat_path<'a>(
    ...
    let levels = dir_levels_up(bin.lhs)?;
    let Expression::Literal(literal::Literal::String(s)) = bin.rhs else {
        return None;
    };
    let rel = if let Some(value) = s.value {
