...
pub(super) fn resolve_rhs_static_call(
        ...
        && let ClassLikeMemberSelector::Identifier(ident) = &static_call.method
    {
        ...
        if let Some(ref owner) = owner {
            let facade_has_method = class_has_method(
                owner,
                &method_name,
                ctx.class_loader,
                ctx.resolved_class_cache,
            );
            // Real Laravel facades declare `getFacadeAccessor()` directly
            // (never inherited), so this raw lookup is a cheap way to rule
            // out the overwhelming majority of static calls that are not on
            // a facade at all, before paying for the source re-parse inside
            // `facade_accessor_concrete_owner` below.
            let is_facade = owner.get_method_ci("getFacadeAccessor").is_some();
            let concrete_owner = if !facade_has_method {
            ...
            {
                // A facade's own method may come from an `@method` docblock
                // tag that flattens the concrete class's argument-dependent
                // return (e.g. `Container::make()`'s `class-string<T>`-aware
                // conditional return becomes a bare `object|mixed` on the
                // `App` facade's tag). Fall through to the concrete
                // accessor only when it actually supplies a conditional
                // return the facade's own tag does not.
                facade_accessor_concrete_owner(
                    owner,
                    &method_name,
                    ctx.content,
