...
What remains is the server-side gate in `code_action` (`src/server.rs`),
which still answers nothing for a template; translating the request range
and its diagnostics into the virtual PHP before the collectors run;
discarding an action whose edits were all dropped rather than offering a
no-op; and the template-aware `use` insertion that B50 in
[`bugs.md`](bugs.md) needs too, shared with completion.
